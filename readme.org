...
* DSDT/SSDT patching based on [[https://www.tonymacx86.com/threads/guide-patching-laptop-dsdt-ssdts.152573/][DSDT/SSDT guide]] and [[https://www.tonymacx86.com/threads/guide-using-clover-to-hotpatch-acpi.200137/][hotpatching guide]]
** Hotpatching
*** Extract patches on running system with [[https://bitbucket.org/RehabMan/os-x-maciasl-patchmatic/downloads/][patchmatic]]
This will "will write all injected DSDT.aml and SSDT*.aml in the order they were injected by Clover" (hotpatching guide).
...
#+BEGIN_SRC sh
  ...
  iasl -da -dl *.aml
#+END_SRC
** Extracting tables for static patching (replaced by hotpatching)
   - Extract with clover: Press F4 on Clover boot screen, then see =EFI/CLOVER/ACPI/origin= for extraced .aml files.
   - Remove =SSDT-4-xh_cfhd4.aml= - contains duplicate definitions for several other files
